...
type GetVersionFn = fn() -> Result<String, Error>;
type GetWebtoonUrlFn = fn(String, String) -> Result<String, Error>;
type GetChapterUrlFn = fn(String, String, String) -> Result<String, Error>;
type GetModulesFn = fn() -> Result<Vec<HashMap<String, Value>>, Error>;
type GetModuleSampleFn = fn(String) -> Result<HashMap<String, String>, Error>;
type GetInfoFn = fn(String, String) -> Result<HashMap<String, Value>, Error>;
type GetChaptersFn = fn(String, String) -> Result<Vec<HashMap<String, String>>, Error>;
type RetrieveImageFn = fn(String, String) -> Result<String, Error>;
type GetImagesFn = fn(String, String, String) -> Result<(Vec<String>, Value), Error>;
type DownloadImageFn = fn(String, String, String) -> Result<Option<String>, Error>;
type SearchByKeywordFn =
    fn(String, String, bool, f64, u32) -> Result<Vec<HashMap<String, String>>, Error>;

pub fn load_modules(modules_path: &PathBuf) -> Result<(), Error> {
...
pub fn unload_modules() -> Result<(), Error> {
    let mut guard: MutexGuard<'_, Option<Library>> =
        LIB.lock()
            .map_err(|err: PoisonError<MutexGuard<'_, Option<Library>>>| {
                Error::lock_library(err.to_string())
...
fn with_symbol<T, F, R>(name: &str, f: F) -> Result<R, Error>
where
    F: FnOnce(Symbol<T>) -> R,
{
...
pub fn get_modules_version() -> Result<String, Error> {
    with_symbol("get_version", |f: Symbol<'_, GetVersionFn>| f())?
}

pub async fn get_modules() -> Result<Vec<HashMap<String, Value>>, Error> {
    with_symbol("get_modules", |f: Symbol<'_, GetModulesFn>| f())?
}

pub async fn get_webtoon_url(domain: String, url: String) -> Result<String, Error> {
...
pub async fn get_info(domain: String, url: String) -> Result<HashMap<String, Value>, Error> {
...
pub async fn retrieve_image(domain: String, url: String) -> Result<String, Error> {
    with_symbol("retrieve_image", |f: Symbol<'_, RetrieveImageFn>| {
        f(domain, url)
    })?
}

pub async fn get_module_sample(domain: String) -> Result<HashMap<String, String>, Error> {
